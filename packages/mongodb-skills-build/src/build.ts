...
import { RULES_DIR, METADATA_FILE, OUTPUT_FILE, SKILL_DIR, getSkillName } from './config.js'
...
function generateMarkdown(
  ...
  md += `> generating, or reviewing MongoDB schemas and queries. Humans may also  \n`
    ...
    section.rules.forEach((rule) => {
      ...
      rule.examples.forEach((example) => {
        ...
        if (example.code && example.code.trim()) {
          md += `\`\`\`${example.language || 'javascript'}\n`
          md += `${example.code}\n`
          md += `\`\`\`\n\n`
        }
...
async function build() {
  try {
    const skillName = getSkillName()
    console.log(`Building AGENTS.md for ${skillName}...`)
    ...
    try {
      ...
      metadata = {
        version: '1.0',
        organization: 'MongoDB Engineering',
        date: new Date().toLocaleDateString('en-US', {
          month: 'long',
          year: 'numeric',
        }),
        abstract:
          'Best practices guide for MongoDB schema design, query optimization, and indexing strategies.',
      }
    }
