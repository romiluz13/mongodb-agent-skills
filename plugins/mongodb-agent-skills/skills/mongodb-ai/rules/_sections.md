...
**Impact:** MEDIUM
**Description:** Hybrid search combines vector (semantic) search with traditional text (lexical) search using $rankFusion or $scoreFusion. This captures both conceptual similarity and exact keyword matches. $rankFusion (MongoDB 8.0+) uses Reciprocal Rank Fusion to merge result lists by position. $scoreFusion (MongoDB 8.2+) merges by actual score values with normalization options (sigmoid, minMaxScaler) and custom combination expressions, offering more granular control for applications where score magnitudes matter. For top-k precision-sensitive use cases, use retrieval (vector/fusion) followed by a reranker in application/provider logic. Treat fusion behavior as release-sensitive and verify on your target patch line. Key constraints include serial sub-pipeline execution, same-collection scope (use $unionWith for cross-collection), limited allowed stages, and pagination caveats. Weights should be tuned per query family rather than fixed globally.
