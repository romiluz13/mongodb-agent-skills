...
**I recommend avoiding $unwind on large arrays or filtering before $unwind, but please verify first:**

| Check | Why It Matters | How to Verify |
|-------|----------------|---------------|
| Average array size | Large arrays cause document explosion | `db.collection.aggregate([{$project:{size:{$size:"$arrayField"}}}])` |
| Max array size | Worst case memory estimation | `db.collection.aggregate([{$group:{_id:null,max:{$max:{$size:"$arrayField"}}}}])` |
| Document count | Total docs x array size = explosion factor | `db.collection.countDocuments()` |

**Verification query:**
```javascript
// Calculate $unwind explosion risk
...
**Interpretation:**
- Good result: Average array size <100 - Safe to $unwind
- Warning result: Average 100-1000 - Filter array before $unwind
- Bad result: Average >1000 or unbounded - Use array operators instead ($size, $filter, $reduce)
...
**What I'll check:**
- `mcp__mongodb__aggregate` - Calculate array size statistics
- `mcp__mongodb__collection-schema` - Identify array fields and typical sizes
- `mcp__mongodb__explain` - Check if $unwind causes disk spills

**Just ask:** "Can you check the array sizes in [collection].[arrayField] before I use $unwind?"
