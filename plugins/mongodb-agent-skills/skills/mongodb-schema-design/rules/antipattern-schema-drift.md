...
**I recommend adding schema validation, but please verify your current schema consistency first:**

| Check | Why It Matters | How to Verify |
|-------|----------------|---------------|
| Find all field variations | Reveals extent of drift | Run field combination aggregation |
| Check for type inconsistencies | Same field with different types breaks queries | Sample documents for type analysis |
| Identify missing required fields | Documents without expected fields cause crashes | Query for missing fields |
| Review existing validation | May already have partial rules | Check collection options |

**Verification query:**
```javascript
// Find all unique field combinations (reveals schema drift)
...
// Multiple results with different field sets = schema drift
```

**Interpretation:**
- Good result (1-2 variations): Schema is consistent, add validation to prevent future drift
- Warning (3-5 variations): Moderate drift, plan migration before adding strict validation
- Bad result (> 5 variations): Significant drift, use `validationLevel: "moderate"` during cleanup
...
**What I'll check:**
- `mcp__mongodb__collection-schema` - Infer current schema structure from samples
- `mcp__mongodb__aggregate` - Detect field variations and type inconsistencies
- `mcp__mongodb__find` - Sample documents to identify drift patterns

**Just ask:** "Can you analyze my collection for schema drift and show me what validation rules I should add?"
