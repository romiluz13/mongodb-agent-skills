...
**I recommend limiting array sizes, but please verify your actual array sizes first:**

| Check | Why It Matters | How to Verify |
|-------|----------------|---------------|
| Find largest arrays | Identifies which collections need refactoring | Run array size aggregation |
| Check array growth rate | Determines urgency of fix | Compare document sizes over time |
| Verify multikey index sizes | Large arrays bloat indexes | Run `db.collection.stats().indexSizes` |
| Assess write frequency | High-frequency writes to large arrays compound the problem | Review application write patterns |

**Verification query:**
```javascript
// Find documents with large arrays (replace 'items' with your array field)
db.collection.aggregate([
  { $project: {
    arraySize: { $size: { $ifNull: ["$items", []] } },
    docSize: { $bsonSize: "$$ROOT" }
  }},
  { $match: { arraySize: { $gt: 100 } } },
  { $sort: { arraySize: -1 } },
  { $limit: 10 }
])
```
...
**Interpretation:**
- Good result (arrays < 100): Safe to embed, no action needed
- Warning (100-500 elements): Use $slice, monitor growth
- Bad result (> 500 elements): Refactor to separate collection immediately
...
**What I'll check:**
- `mcp__mongodb__aggregate` - Find documents with largest arrays
- `mcp__mongodb__collection-indexes` - Check multikey index sizes
- `mcp__mongodb__collection-schema` - Identify array fields in your schema

**Just ask:** "Can you check my collection for large arrays and recommend which ones to refactor?"
