...
**I recommend pre-computing values, but please verify your computation needs first:**

| Check | Why It Matters | How to Verify |
|-------|----------------|---------------|
| Find expensive aggregations | Candidates for pre-computation | Profile slow queries |
| Check aggregation frequency | Frequently-run = high benefit from caching | Count executions |
| Measure computation time | Long-running aggregations benefit most | Check query duration |
| Verify update frequency | Frequently-changing source data complicates updates | Review write patterns |

**Verification query:**
```javascript
// Find aggregations that should be pre-computed
db.setProfilingLevel(1, { slowms: 100 })
...
db.system.profile.aggregate([
  { $match: { "command.aggregate": { $exists: true } } },
  { $group: {
    _id: { $substr: [{ $toString: "$command.pipeline" }, 0, 200] },
    count: { $sum: 1 },
    avgMs: { $avg: "$millis" },
    maxMs: { $max: "$millis" }
  }},
...
**Interpretation:**
- Good result (no repeated slow aggregations): Pre-computation not needed
- Warning (aggregations run 10-100 times/day, > 100ms): Consider pre-computing
- Bad result (aggregations run 100+ times/day, > 500ms): Pre-compute immediately
...
**What I'll check:**
- `mcp__mongodb__aggregate` - Identify expensive computations in your queries
- `mcp__mongodb__explain` - Analyze query execution plans
- `mcp__mongodb__find` - Check if computed fields already exist

**Just ask:** "Can you analyze my queries and identify which computations should be pre-calculated and stored?"
