...
**I recommend extended references, but please verify your lookup patterns first:**

| Check | Why It Matters | How to Verify |
|-------|----------------|---------------|
| Find frequent $lookup targets | Collections always looked up = embed candidates | Profile $lookup queries |
| Identify display-only fields | Fields only used for display should be cached | Review UI data needs |
| Check source data change rate | Frequently-changing data complicates sync | Review update patterns |
| Measure $lookup latency | High latency justifies caching complexity | Profile query times |

**Verification query:**
```javascript
// Find collections that are frequently $lookup targets
db.setProfilingLevel(1, { slowms: 20 })
// ... run your application ...

db.system.profile.aggregate([
  { $match: { "command.pipeline": { $elemMatch: { "$lookup": { $exists: true } } } } },
  { $unwind: "$command.pipeline" },
  { $match: { "$command.pipeline.$lookup": { $exists: true } } },
  { $group: {
    _id: "$command.pipeline.$lookup.from",
    count: { $sum: 1 },
    avgMs: { $avg: "$millis" }
  }},
  { $sort: { count: -1 } }
])
// High count = candidate for extended reference pattern
```

**Interpretation:**
- Good result (few $lookups, < 20ms): Current references are fine
- Warning ($lookups adding 20-50ms per query): Consider caching frequently-used fields
- Bad result ($lookups adding > 50ms, high frequency): Implement extended reference pattern
...
**What I'll check:**
- `mcp__mongodb__collection-schema` - Identify reference fields (ending in Id)
- `mcp__mongodb__find` - Sample documents to see current reference structure
- `mcp__mongodb__aggregate` - Analyze $lookup patterns and latency

**Just ask:** "Can you analyze my references and recommend which fields should be cached using the extended reference pattern?"
